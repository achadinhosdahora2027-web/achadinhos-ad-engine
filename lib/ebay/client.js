...
exports.EbayClient = exports.EBAY_ENDPOINTS = void 0;
exports.ebayDeepLink = ebayDeepLink;
exports.EBAY_ENDPOINTS = {
    production: {
...
class EbayClient {
    ...
    hasUsableCredentials() {
        return Boolean(this.creds.credentialsVerified &&
    ...
    async getToken(scopes = ['https://api.ebay.com/oauth/api_scope']) {
        const t0 = Date.now();
        ...
        const endpoint = exports.EBAY_ENDPOINTS[this.env].oauth;
        ...
        try {
            ...
            return {
                ok: false, status: 0, durationMs: Date.now() - t0,
                error: String(e.message || e), telemetry: 'Sintonizado em Análise',
            };
        }
    }
    /**
     ...
     */
    async merchandisedProducts(categoryId, metric = 'BEST_SELLING') {
        const t0 = Date.now();
        ...
        const url = `${exports.EBAY_ENDPOINTS[this.env].marketing}?category_id=${encodeURIComponent(categoryId)}&metric_name=${metric}`;
        try {
            ...
            return { ok: false, status: 0, durationMs: Date.now() - t0, error: String(e.message || e), telemetry: 'Sintonizado em Análise' };
        }
    }
}
 ...
 */
function ebayDeepLink(itemUrl, opts) {
    if (!/^\d{10}$/.test(opts.campaignId))
        return null;
    try {
        ...
        if (opts.sid)
            u.searchParams.set('customid', String(opts.sid).slice(0, 60));
        if (opts.rover)
            u.searchParams.set('mpre', 'https://www.ebay.com');
        return u.toString();
    }
    catch {
        return null;
