...
/* v340.0 - ALIAS DE TIPO. O publicador 24/7 pedia kind='deal' enquanto o registro
   declara 'publish': o filtro devolvia ZERO destinos e o workflow autonomo morria
   com exit 1 ("Nenhum destino ativo com chat_id") a cada ciclo, mesmo com tres
   destinos resolvidos. Agora 'deal' casa com quem recebe publicacao. */
const KIND_ALIASES = {
  deal: ['publish', 'deal'],
  oferta: ['publish', 'deal'],
  offers: ['publish', 'deal'],
...
function activeDestinations(reg, kind) {
  return (reg.destinations || []).filter((d) => {
    if (d.enabled === false || !d.chat_id) return false;
    if (!kind) return true;
    const recebe = d.receives || [];
    const aceitos = KIND_ALIASES[kind] || [kind];
    return aceitos.some((k) => recebe.includes(k));
  });
}
