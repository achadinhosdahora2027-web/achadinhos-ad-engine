...
export const VERSAO = 'v340.0';

/** Núcleo do swap internacional — estritamente cinco países. */
export const TIER1_CORE = ['US', 'CA', 'GB', 'DE', 'FR'] as const;

/** Merchants que cobram em moeda estrangeira do visitante brasileiro. */
export const MOEDA_ESTRANGEIRA = [
  'booking', 'booking_uk', 'booking_latam', 'ebay', 'ebay_us', 'amazon', 'amazon_us',
  'aliexpress', 'udemy', 'nordvpn', 'economybookings', 'brunoyam',
] as const;

export const SLOTS_SINTETICOS = ['_health_desktop', 'health', '_health', 'health_desktop'] as const;

export const HOSTS_MOEDA_ESTRANGEIRA =
  /(^|\.)(booking\.com|ebay\.(com|co\.uk|de|fr|it|es|ca|com\.au)|amazon\.(com|co\.uk|de|fr|es|it|ca)|aliexpress\.com|udemy\.com|nordvpn\.com|economybookings\.com|brunoyam\.com)$/i;
export const HOSTS_CJ_INTERNACIONAIS = /(kqzyfj|jdoqocy|dpbolvw|anrdoezrs|tkqlhce)\.(com|net)/i;
...
export const LINKS = {
  ...
  ebayEPN:
    'https://www.ebay.com/deals?campid=5339193749&toolid=10001&mkevt=1&mkcid=1&mkrid=711-53200-19255-0',
} as const;

export type Arquitetura = 'desktop' | 'mobile' | 'tablet' | 'bot' | 'desconhecido';
export type DecisaoGeo =
  | 'br_lock_moeda_estrangeira'
  | 'br_nacional'
  | 'tier1_swap_exclusivo'
  | 'regional'
  | 'sintonizado_em_analise';
...
export function slugKeyword(texto: string | null | undefined): string {
  return String(texto || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export function slotDinamico(keyword: string | null | undefined, userAgent: string | undefined): string {
  const arch = arquiteturaUA(userAgent);
  const kw = slugKeyword(keyword);
  if (!kw || /^[0-9]+$/.test(kw) || (SLOTS_SINTETICOS as readonly string[]).includes(kw)) {
...
export function resolverCountry(e: EntradaRota): { country: string; fonte: string } {
  const h = e.headers || {};
  const doIp = h['x-vercel-ip-country'] || h['cf-ipcountry'] || h['x-country-code'];
  if (doIp && doIp.length >= 2) return { country: doIp.toUpperCase().slice(0, 2), fonte: 'ip_real' };
  const hint = String(e.geoHint || '').toUpperCase().replace(/[^A-Z]/g, '').slice(0, 2);
  if (hint && (e.geoforce || process.env.GEO_PARAM_ALLOW === '1')) return { country: hint, fonte: 'dica_borda' };
  if (hint && !doIp) return { country: hint, fonte: 'parametro_sem_ip' };
  return { country: 'BR', fonte: 'default_br' };
}

export function destinoMoedaEstrangeira(url: string | null | undefined): boolean {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase();
    if (HOSTS_CJ_INTERNACIONAIS.test(host)) return true;
    return HOSTS_MOEDA_ESTRANGEIRA.test(host);
  } catch {
    return false;
  }
}
 ...
 */
export function marcarLink(url: string, sid: string, country: string): string {
  try {
    const u = new URL(url);
    ...
    if (u.hostname.includes('shopee')) u.searchParams.set('sub_id', sid);
    if (u.hostname.includes('ebay')) u.searchParams.set('customid', sid);
    return u.toString();
  } catch {
...
import { resolveShopeeOffer } from './shopee-catalog';
 ...
 */
export function resolverRota(entrada: EntradaRota, campanhaId = '5339193749', pidCj = '101859672'): SaidaRota {
  ...
  try {
    ...
    if (country === 'BR' && !isBot) {
      ...
      const precisaBloquear = (MOEDA_ESTRANGEIRA as readonly string[]).includes(brand);
      const oferta = entrada.keyword ? resolveShopeeOffer({ q: entrada.keyword }) : null;
      if (precisaBloquear) {
        const link = oferta?.u || LINKS.meliLa;
        return {
          versao: VERSAO, country, brand: oferta ? 'shopee' : 'mercadolivre',
      ...
      const linkBr = oferta?.u || LINKS.shopeeBR;
      return {
        versao: VERSAO, country, brand: oferta ? 'shopee' : brand || 'shopee',
    ...
    if ((TIER1_CORE as readonly string[]).includes(country)) {
      if (brand === 'auto' || !brand) brand = 'ebay';
      if (brand === 'booking') brand = 'booking_uk';
      const link =
        brand === 'ebay' && !entrada.offerHash
          ? LINKS.ebayEPN.replace(/campid=\d+/, `campid=${campanhaId}`)
          : brand === 'booking_uk'
            ? LINKS.bookingUK.replace('{PID}', pidCj)
            : null;
      return {
        versao: VERSAO, country, brand, decisao: 'tier1_swap_exclusivo', isBot,
        ...
        failClosed: false, motivo: `Tier-1 núcleo (${TIER1_CORE.join('/')})`,
      };
    }

    /* ── 3) DEMAIS PAÍSES: rota regional, sem swap indevido ────────────── */
    return {
      versao: VERSAO, country, brand: brand === 'auto' || !brand ? 'aliexpress' : brand,
    ...
    return {
      versao: VERSAO, country, brand: 'mercadolivre', decisao: 'sintonizado_em_analise',
      isBot, bloqueouMoedaEstrangeira: false, tier1: false,
      slotDinamico: slotDinamico(entrada.keyword, ua), slotOrigem, arquitetura,
      link: LINKS.meliLa, failClosed: true, motivo: String((e as Error)?.message || e).slice(0, 160),
    };
  }
}

/* ───────────────────── mídia programática (por host) ────────────────────── */
 ...
 */
export function planoMidia(host: string, mapa: Record<string, { pop?: string | null; sb?: string | null }>): PlanoMidia {
...
export function scriptCreateElement(tag: string | null, zona: string | null = null): string {
  if (!tag) return '';
