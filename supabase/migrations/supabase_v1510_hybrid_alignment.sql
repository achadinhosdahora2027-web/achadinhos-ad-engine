...
--   * Jobs 15, 16, 18, 62, 64 and 65 stay OFF. Job 60 is replaced by source-event triggers;
...
select cron.alter_job(62,active:=false) where exists(select 1 from cron.job where jobid=62);
select cron.alter_job(64,active:=false) where exists(select 1 from cron.job where jobid=64);
select cron.alter_job(65,active:=false) where exists(select 1 from cron.job where jobid=65);
...
select
  ...
  not exists(select 1 from cron.job where jobid in(15,16,18,60,62,64,65) and active) as prohibited_polling_jobs_off,
