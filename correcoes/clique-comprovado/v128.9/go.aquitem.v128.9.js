...
module.exports = async (req, res) => {
  ...
  /* v128.9 — SUBORIGEM REAL: em slot genérico (jetstream/header/inline/...), a
     suborigem do clique passa a ser o PRODUTO real (kw/q), nunca o nome da
     campanha. O slot declarado por CTA de cidade é preservado (atribuição). */
  const SLOT_GENERICO_ = /^(jetstream|header|inline|health|sem_keyword|radar|city|ci|lab|fanout|digest|v\d{2,3})/i.test(slot);
  const kwProduto_ = String(query.kw || query.q || query.keyword || '').toLowerCase()
    .replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 40);
  const slotParaSid_ = (SLOT_GENERICO_ && kwProduto_) ? kwProduto_ : slot;
  const sid = query.sid || `${site}_${country.toLowerCase()}_${slotParaSid_}${String(slotParaSid_).toLowerCase().endsWith(String(device).toLowerCase()) ? '' : '_' + device}`;
  ...
  let TOKEN_CLIQUE = '';   // preenchido mais abaixo, quando o destino final já existe
  ...
  /* v128.8 — INSERT em ads_clicks REMOVIDO daqui. Era este trecho que gravava
  ...
  res.setHeader('X-Affiliate-Engine', 'Achadinhos-Global-Gateway-2026-v128.8');
  ...
  const TAG_SOCIALBAR_EF = MOSTRAR_TAGS ? TAG_SOCIALBAR : null;
  const DWELL_MS = 4000;
  ...
  /* v128.8 — token do clique comprovado: montado AQUI, depois de o destino
     final existir (antes ficava cedo demais e a assinatura falhava em silêncio). */
  if (CONTA_CLIQUE) {
    try {
